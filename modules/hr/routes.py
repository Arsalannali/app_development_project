"""
================================================================================
HR MODULE - Routes and Business Logic
================================================================================

This module contains all HR-related routes and functionality including:
- Employee Management
- Attendance Tracking
- Leave Management
- Payroll Processing
...
# Create HR Blueprint
hr_bp = Blueprint('hr', __name__, template_folder='../../templates/hr', url_prefix='/hr')
...
EMPLOYEES_FILE = os.path.join(HR_DATA_DIR, 'employees.json')
USERS_FILE = os.path.join(DATA_DIR, 'users.json')
DEPARTMENTS_FILE = os.path.join(HR_DATA_DIR, 'departments.json')
ATTENDANCE_FILE = os.path.join(HR_DATA_DIR, 'attendance.json')
LEAVES_FILE = os.path.join(HR_DATA_DIR, 'leaves.json')
PAYROLLS_FILE = os.path.join(HR_DATA_DIR, 'payrolls.json')
JOBS_FILE = os.path.join(HR_DATA_DIR, 'jobs.json')
APPLICANTS_FILE = os.path.join(HR_DATA_DIR, 'applicants.json')
...
def dashboard():
    ...
    if not session.get('user_id'):
        flash('Please log in to access the HR module.', 'warning')
        return redirect(url_for('login'))
    ...
    today_attendance = len([a for a in attendance_records if a.get('date') == datetime.now().strftime('%Y-%m-%d')])
...
# Login route removed - using global login instead
...
def attendance_list():
    ...
    if current_role == 'Employee':
        attendance_records = [r for r in attendance_records if r.get('employee_id') == current_employee_id]
...
def attendance_mark():
    ...
    if request.method == 'POST':
        ...
        if current_role == 'Employee' and employee_id != current_employee_id:
            ...
            return redirect(url_for('hr.attendance_mark'))
        ...
        if action == 'check_in':
            ...
            elif today_record.get('check_out_time'):
...
@hr_bp.route('/attendance/reports')
...
def attendance_reports():
    ...
    if employee_id:
        filtered_records = [r for r in filtered_records if r.get('employee_id') == int(employee_id)]
    
    # Add employee names
    employee_lookup = {emp['employee_id']: emp for emp in employees}
    for record in filtered_records:
        emp = employee_lookup.get(record.get('employee_id'))
        record['employee_name'] = f"{emp['first_name']} {emp['last_name']}" if emp else "Unknown"
...
@hr_bp.route('/attendance/edit/<int:attendance_id>', methods=['GET', 'POST'])
@role_required('Admin', 'HR Staff')
def attendance_edit(attendance_id):
    """Edit an attendance record."""
    attendance_records = read_json_file(ATTENDANCE_FILE)
    attendance = next((a for a in attendance_records if a.get('attendance_id') == attendance_id), None)
...
@hr_bp.route('/attendance/delete/<int:attendance_id>', methods=['POST'])
@role_required('Admin', 'HR Staff')
...
def leaves_list():
    ...
    if current_role == 'Employee':
        leaves = [l for l in leaves if l.get('employee_id') == current_employee_id]
    elif current_role not in ['Admin', 'HR Staff']:
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('hr.dashboard'))
    ...
    for leave in leaves:
        emp = employee_lookup.get(leave.get('employee_id'))
...
@hr_bp.route('/leaves/approve/<int:leave_id>', methods=['POST'])
@role_required('Admin', 'HR Staff')
...
@hr_bp.route('/leaves/reject/<int:leave_id>', methods=['POST'])
@role_required('Admin', 'HR Staff')
...
@hr_bp.route('/leaves/edit/<int:leave_id>', methods=['GET', 'POST'])
@login_required
...
@hr_bp.route('/leaves/delete/<int:leave_id>', methods=['POST'])
@login_required
...
@hr_bp.route('/employees/view/<int:employee_id>')
@login_required
...
@hr_bp.route('/payroll/view/<int:payroll_id>')
@login_required
...
@hr_bp.route('/payroll/delete/<int:payroll_id>', methods=['POST'])
@role_required('Admin')
def payroll_delete(payroll_id):
    """Delete a payroll record (Admin only)."""
    payrolls = read_json_file(PAYROLLS_FILE)
    payroll = next((p for p in payrolls if p.get('payroll_id') == payroll_id), None)
    
    if payroll:
        payrolls = [p for p in payrolls if p.get('payroll_id') != payroll_id]
        write_json_file(PAYROLLS_FILE, payrolls)
...
def job_add():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('hr.jobs_list'))
    
    departments = read_json_file(DEPARTMENTS_FILE)
    return render_template('jobs/form.html', job=None, departments=departments)

@hr_bp.route('/jobs/edit/<int:job_id>', methods=['GET', 'POST'])
@role_required('Admin', 'HR Staff')
def job_edit(job_id):
...
@hr_bp.route('/jobs/delete/<int:job_id>', methods=['POST'])
@role_required('Admin', 'HR Staff')
...
@hr_bp.route('/jobs/view/<int:job_id>')
def job_view(job_id):
...
@hr_bp.route('/jobs/apply/<int:job_id>', methods=['GET', 'POST'])
def job_apply(job_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('hr.job_view', job_id=job_id))
...
@hr_bp.route('/applicants/view/<int:applicant_id>')
@role_required('Admin', 'HR Staff')
def applicant_view(applicant_id):
    ...
    if not applicant:
        ...
        return redirect(url_for('hr.applicants_list'))
...
@hr_bp.route('/applicants/update-status/<int:applicant_id>', methods=['POST'])
@role_required('Admin', 'HR Staff')
def applicant_update_status(applicant_id):
    ...
    return redirect(url_for('hr.applicant_view', applicant_id=applicant_id))

@hr_bp.route('/applicants/onboard/<int:applicant_id>', methods=['GET', 'POST'])
@role_required('Admin', 'HR Staff')
...
@hr_bp.route('/applicants/download/<filename>')
@role_required('Admin', 'HR Staff')
...
@hr_bp.route('/applicants/delete/<int:applicant_id>', methods=['POST'])
@role_required('Admin')
...
@hr_bp.route('/settings', methods=['GET', 'POST'])
@role_required('Admin', 'HR Staff')
def settings():
